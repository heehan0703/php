...
require_once('wp-admin/include/connectdb.php');
 ...
 if(isset($_POST['email']) && $_POST['email']!='' && $_POST['pass']==$_POST['conf_pass']){ 
  ...
  $ReferralPerson = $_POST['ReferralPerson'];
    ...
    $countryrow=mysql_fetch_array(mysql_query("select * from country where country_Id='$country'"));
	$staterow=mysql_fetch_array(mysql_query("select * from state where state_id='$state'"));
	
	 $numagent=mysql_num_rows(mysql_query("select * from member where i_am='Agent' and state='$state'"));
	 
	 $codenum=str_pad($numagent, 4, '0', STR_PAD_LEFT)
	 
	$countrysub=substr($countryrow['country_name'],0,2);
	$statesub=substr($staterow['state_name'],0,2);
	$lastsocial=substr($social,-4);
  
  $agent_code=$statesub.$codenum;
...
$stmt->bindParam(':state',$state='');	 
 ...
 $from = "fahair <info@fahair.com>";
 $to= "$email";
 //$to = "baislavimal@gmail.com";
 $subject ="[fahair] Order confirmation";
 $body ="<!doctype html>
<html>
<head>
...
<div class=WordSection1>
 ...
 header("location:sign_in_agent.php?email=$email");
 }
  }
else{
