...
//echo "$num";
	...
	mysql_query("insert into commission_detail set 	agent_id='$newagentcode',order_id='$neworderid',processing_date='$newdate',saleamount='$newsaleamount',mypercentage='$newpercentage',pay='$pay',agentname='$newagentname',saleagent='$newsaleagent'");
...
$childrens=array();
//echo "select * from `member` where parent_id='$userid'";
...
function childrenlisttwo($userid){
$childrens=array();
   ...
   $childrens[]=$userid;
		if(mysql_num_rows($querychildren)>0){
			while($rowchildren=mysql_fetch_array($querychildren)){ 
			    $childrens[]=$rowchildren['member_id'];
				//var_dump($childrens);
			    $childrens=array_merge($childrens,childrenlist($rowchildren['parent_id']));
			 }
		}
...
$childrenarraytwo=childrenlisttwo($member_id);

//var_dump($childrenarraytwo);
...
function numtransaction($valnarray,$timestamp){
		...
		//echo "select * from `member` where member_id='$value'";
		     ...
		     $ordernum=mysql_num_rows($order);
				 ...
				 // echo $orderrow['Agent_code'];
				     $numtransaction=$numtransaction+numberoftransaction($value,$orderrow['Agent_code'],$timestamp);
				  }		
 ...
 $transactionsum=0;
		...
		//echo "select * from `member` where member_id='$value'";
		  ...
		  $ordernum=mysql_num_rows($order);
				  if($ordernum){
				 // echo $orderrow['Agent_code'];
				     $transactionsum=$transactionsum+sumcommissiondetail($value,$orderrow['Agent_code'],$timestamp);
				  }		
...
function newweeklydata($valnarray){
...
//echo "$numberofweeks";
		...
		$timestamp=$timestamp-24*60*60*7;
		 //echo date("l jS \of F Y h:i:s A",$timestamp);
		   $weeksum= sumofweek($timestamp,$valnarray);
		   $numberoftransaction=numtransaction($valnarray,$timestamp);
		  ...
		  $transactionvalue.='<tr>  
		   ...
		   if($numberoftransaction){
		  $transactionvalue.=transaction($valnarray,$timestamp);
		  }
		}
...
function weeklydata($valnarray){
$timestamp=time();
if(date("w",$timestamp)==2){
$timestamp=strtotime("this Tuesday");
}else{
		...
		$timestamp=$timestamp-24*60*60*7;
		  ...
		  $transactionvalue.='<tr>  
		   ...
		   $transactionvalue.='<tr style="background-color:#eeeeee" > 
                                       ...
                                       <td style="width:100px; text-align:center;font-size:14px">Sales Amount</td>   
                                       <td style="width:100px; text-align:center;font-size:14px">My%</td> 
                                       <td style="width:100px; text-align:center;font-size:14px" colspan="2">Pay</td> 
                                   </tr>';
			 }
		   if($numberoftransaction){
		  $transactionvalue.=transaction($valnarray,$timestamp);
...
$startime=$timestamp+24*60*60*7;
$endtime=$timestamp;
//echo "start". date('m-d-Y', $startime);
//echo "<br/>";
//echo "end". date('m-d-Y', $endtime);
...
function searchtransaction($valnarray,$startdate,$enddate){
  ...
  //echo "dhirendra";
        $pendingform='<tr style="background-color:#eeeeee" > 
                                       <td style="width:100px; text-align:center;font-size:14px">Order#</td>  
                                       <td style="width:100px; text-align:center;font-size:14px">Processed Date</td>  
                                       <td style="width:100px; text-align:center;font-size:14px">Sale Agent</td>  
                                       <td style="width:100px; text-align:center;font-size:14px">Agent</td>  
...
function currentyearcommition($user){
	...
	//echo "select * from commission_detail where processing_date>=$year and agent_id='$user'";
             ...
             <tr>  
                  <td bgcolor="#FFFFFF" style=""> 
                        <div style="float:left; padding-bottom:5px; width:200px; background-color:#56af45; "> 
                             ...
                             <div style="float:left;margin-bottom:10px; margin-top:10px;"> <span style="color:#FFFFFF">
                             <font size="+1">$<?=number_format(currentyearcommition($query['Agent_code']),2);?> </font> <br/>  Commission Paid </span> </div>
            ...
            <tr>  
                  <td bgcolor="#FFFFFF" style="border-top:none"> 
                     <table style="width:100%;">
                        ...
                        <?=newweeklydata($childrenarraytwo)?>
                     </table> 
                  </td>
            </tr>
                    ...
                    <table style="width:100%;">
                     <tr> <td bgcolor="#FFFFFF">
                               ...
                               <table style="width:100%;">
								   ...
								   ?>
                              <?  if($_POST['search']){ ?>
                              <?=searchtransaction($childrenarraytwo,$startdate,$enddate)?>
                              <? }else{ ?>
                               <?=weeklydata($childrenarraytwo)?>
                               <? } ?>
                               </table> 
