...
$message_query=mysql_query(dopaging(" SELECT * FROM `message` where sent=1 order by id desc ",10));
                    ...
                    <td width="7%"><?=$message_row['user_type']?></td>  
                       <td width="7%"><?=$message_row['from']?></td>  
                          <td width="18%"><?=$message_row['subject']?></td>  
                             <td width="" style=" width:400px;"><pre style="width:400px;"><?=$message_row['message_detail']?></pre></td>                  <td width="5%"><a href="message_replay.php?id=<?=$user_email?>&subject=<?=$message_row['subject']?>">Reply</a></td>
