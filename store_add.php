...
<script src="//maps.googleapis.com/maps/api/js?v=3.exp&sensor=false&key=AIzaSyB0FmqpdpeQ_QGHTJ5q8U-nzQT0dSvjQsY" async="" defer="defer" type="text/javascript"></script>
...
<div class="full" style="background-color:white;">
  ...
  <a style="color:inherit;"> <?= $store['s_name'];?></a>
     ...
     <div class="row" style="padding:1em;">
     <a style="color:inherit;"> <H1><b><?= $store['s_name'];?></H1></b></a>
     </div>
  </div>
  ...
  <div class="container">
    <div class="row" style="padding: 1em 10% 1em 10%;"> 
      ...
      <ul class="rslides" id="slider4">
        <?php 
		  $add=mysql_query("SELECT * FROM `adds` where store_id='$getid' ");
	 
	// echo "select * from adds where store_id='$getid'";
	  while($adds=mysql_fetch_assoc($add)){
		
		?>
        <li>
          <img src="./storeadsimages/<?= $adds['add_image'];?>" alt="">
          <p class="caption1"><?= $adds['add_name'];?></p>
        </li>
         <?php }?>
      </ul>
